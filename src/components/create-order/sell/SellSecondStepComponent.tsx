...
export const SellSecondStepComponent: React.FC<
  SellSecondStepComponentProps
> = ({ data, setData, setStep, setOrderData }) => {
  ...
  return (
    <Container>
      <StyledImage src="/svg/img/progressbar-sell-02.png" alt="progress-bar" />
      <Title>
        입금하실 모빅 수는 <BlueText>{data.amount}개</BlueText> 입니다.
      </Title>
      <Description>
        희망 가격 : {data.price}원 * <BlueText>&nbsp;{data.amount}</BlueText>개
      </Description>
      <Description>
      ...
      <Instruction>
        (장이 종료되고 반환은 이 주소를 통해 진행됩니다.)
      </Instruction>
      <Form>
        <Label>
          <BlueText>{data.nickname}</BlueText> 님의 주소
        </Label>
        <Input
      ...
      <Instruction>매칭이 완료되면 아래 계좌로 입금됩니다.</Instruction>
      <Form>
        <Label>은행명</Label>
        <Input
          name="bankName"
